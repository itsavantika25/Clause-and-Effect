      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            {[
              ...
              <button key={id} onClick={() => scrollToSection(id)}
                className="block text-left text-slate-400 hover:text-[#fdd25c] transition-colors font-body text-sm uppercase tracking-widest">
                {label}
              </button>
            ))}
          </div>
          <div className="space-y-4">
            ...
            <button onClick={() => window.open('https://www.instagram.com/clauseandeffect_', '_blank')}
              className="block text-left text-slate-400 hover:text-[#fdd25c] transition-colors font-body text-sm uppercase tracking-widest">Instagram</button>
            <button onClick={() => window.open('https://www.youtube.com/@clauseandeffect57', '_blank')}
              className="block text-left text-slate-400 hover:text-[#fdd25c] transition-colors font-body text-sm uppercase tracking-widest">YouTube</button>
          </div>
        </div>
      </div>
    </footer>
...
const Landing = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setReels(snapshot.docs.map(d => ({ id: d.id, ...d.data() })) as Reel[]);
    });
    return () => unsubscribe();
  }, []);
    ...
    <main className="pt-20">
      ...
      <section id="hero" className="relative min-h-[921px] flex items-center overflow-hidden px-8">
        <div className="absolute inset-0 z-0 bg-[#050a18]">
          <div className="absolute inset-0 opacity-10" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1589829545856-d10d557cf95f?auto=format&fit=crop&q=80&w=1920&h=1080')" }} />
          <div className="absolute top-1/4 -right-20 w-96 h-96 bg-[#fdd25c]/10 rounded-full blur-[120px]" />
          <div className="absolute bottom-1/4 -left-20 w-96 h-96 bg-[#fdd25c]/10 rounded-full blur-[120px]" />
        </div>
        <div className="relative z-10 max-w-7xl mx-auto w-full grid md:grid-cols-2 gap-12 items-center">
          <motion.div variants={slideLeft} initial="hidden" animate="visible" className="space-y-8">
            <span className="inline-block px-4 py-1.5 rounded-full bg-[#fdd25c]/10 text-[#fdd25c] text-xs font-bold tracking-[0.2em] uppercase">
              Asking the Questions Others are too Scared to Think Of.
            </span>
            ...
            <div className="flex items-center gap-6 pt-4">
              ...
              <button
                ...
                className="px-8 py-4 text-white border border-white/20 rounded-xl font-bold text-lg backdrop-blur-sm hover:bg-white/10 transition-colors"
              >
                Our Vision
              </button>
            </div>
          ...
          <div className="relative hidden md:block">
            <motion.div variants={scaleIn} initial="hidden" animate="visible"
              ...
              <img className="w-full h-full object-cover" alt="abstract architectural shot"
      ...
      <section className="py-32 px-8 bg-white" id="about">
        <div className="max-w-7xl mx-auto">
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="brief-accent mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            <motion.div variants={slideLeft} initial="hidden" whileInView="visible" viewport={{ once: true }}
              className="md:col-span-7 bg-gray-50 p-12 rounded-3xl shadow-sm flex flex-col justify-between">
              <div>
            ...
            <motion.div variants={slideRight} initial="hidden" whileInView="visible" viewport={{ once: true }}
              className="md:col-span-5 bg-[#050a18] p-12 rounded-3xl text-white relative overflow-hidden">
            ...
            {[
              ...
              <motion.div key={stat.label} custom={i} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}
                ...
                <div>
                  <div className={`text-5xl font-headline font-extrabold ${stat.textColor} mb-2`}>{stat.num}</div>
                  <div className={`text-sm font-label uppercase tracking-widest ${stat.labelColor}`}>{stat.label}</div>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-24 bg-gray-50 overflow-hidden">
        <div className="max-w-7xl mx-auto px-8 mb-12 flex justify-between items-end">
          ...
          <motion.div variants={fadeIn} initial="hidden" whileInView="visible" viewport={{ once: true }} className="flex gap-2">
            <button onClick={() => scrollCarousel('left')} className="p-3 bg-white rounded-full shadow-sm hover:bg-slate-50 transition-colors">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <button onClick={() => scrollCarousel('right')} className="p-3 bg-white rounded-full shadow-sm hover:bg-slate-50 transition-colors">
              <ArrowRight className="w-5 h-5" />
            </button>
          </motion.div>
        </div>
        <div ref={carouselRef} className="flex gap-6 px-8 overflow-x-auto no-scrollbar pb-8">
          {reels.map((reel, i) => (
            <motion.div
              key={reel.id}
              custom={i}
      ...
      <section id="team" className="py-32 bg-white">
        <div className="max-w-7xl mx-auto px-8">
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-20">
            ...
            <div className="w-24 h-1 bg-[#fdd25c] mx-auto" />
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
            {team.map((member, i) => (
              <motion.div key={member.name} custom={i} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="group">
      ...
      <section id="contact" className="py-20 px-4 sm:px-6 md:py-32 md:px-8 bg-[#050a18] text-white">
        <div className="max-w-7xl mx-auto text-center">
          <motion.div variants={fadeIn} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <div className="inline-block w-16 sm:w-24 h-1 bg-[#fdd25c] mb-8 sm:mb-12" />
            <h2 className="text-3xl sm:text-5xl md:text-7xl font-headline font-bold mb-6 sm:mb-8">Contact Us, 24/7</h2>
            <p className="text-base sm:text-lg md:text-xl text-slate-400 max-w-2xl mx-auto mb-12 sm:mb-16 px-2">
              Feel free to reach out anytime for legal advice, debates or just to talk. DM us on Instagram, or email for formal queries.
            </p>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 max-w-4xl mx-auto">
            {[
              { href: 'https://instagram.com/clauseandeffect_', icon: <Globe className="w-6 h-6 sm:w-8 sm:h-8" />, label: 'Instagram', value: '@clauseandeffect_', target: '_blank' as const },
              { href: 'mailto:clause.and.effect57@gmail.com', icon: <Send className="w-6 h-6 sm:w-8 sm:h-8" />, label: 'Email', value: 'clause.and.effect57@gmail.com', target: undefined },
            ].map((item, i) => (
              <motion.a key={item.label} custom={i} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}
                ...
                <h3 className="text-xl sm:text-2xl font-headline font-bold mb-1 sm:mb-2">{item.label}</h3>
                <p className="text-[#fdd25c] font-bold text-sm sm:text-base break-all">{item.value}</p>
              </motion.a>
            ))}
          </div>
        </div>
        ...
        {selectedReel && (
          ...
          >
            ...
            >
              ...
              {isInstagramUrl(selectedReel.video) ? (
                <iframe
                  src={getInstagramEmbedUrl(selectedReel.video) ?? selectedReel.video}
...
const PostDetail = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, 'posts', id), (snap) => {
      snap.exists() ? setPost({ id: snap.id, ...snap.data() } as Post) : navigate('/');
      setLoading(false);
    });
    return () => unsubscribe();
  }, [id, navigate]);
  ...
  return (
    <div className="pt-20 pb-40">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div variants={fadeUp} initial="hidden" animate="visible" className="pt-12">
          <Link to="/blogs" className="inline-flex items-center space-x-2 text-sm font-bold text-gray-500 hover:text-black mb-12 transition-colors">
            <ArrowLeft className="w-4 h-4" /><span>Back to Blogs</span>
          </Link>
          <div className="flex items-center space-x-3 mb-8">
            <span className="text-gray-400 text-sm">{post.readingTime}</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-serif font-bold leading-tight mb-12">{post.title}</h1>
          <div className="flex items-center space-x-6 mb-16 pb-8 border-b border-gray-100">
            <div className="flex items-center space-x-4">
              <img src={post.authorImage} alt={post.authorName}
                className="w-12 h-12 rounded-full object-cover" referrerPolicy="no-referrer" />
              <div>
...
const AdminDashboard = ({ user }: { user: FirebaseUser | null }) => {
  ...
  const [currentReel, setCurrentReel] = useState<Partial<Reel>>({ title: '', views: '', img: '', video: '', order: 0 });

  useEffect(() => {
    const unsub = onSnapshot(query(collection(db, 'posts'), orderBy('publishedAt', 'desc')), (snap) => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(query(collection(db, 'reels'), orderBy('order', 'asc')), (snap) => {
  ...
  const emptyPost = (): Partial<Post> => ({
    ...
    authorName: user?.displayName || '', authorRole: 'Contributor',
    authorImage: user?.photoURL || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=100&h=100',
    coverImage: 'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?auto=format&fit=crop&q=80&w=1200&h=800',
    readingTime: '5 min read', status: 'published', publishedAt: new Date().toISOString(), views: 0,
  });
  ...
  const handleSaveReel = async (e: React.FormEvent) => {
    ...
    try {
      if (currentReel.id) {
        const { id: _id, ...data } = currentReel as Reel;
        await updateDoc(doc(db, 'reels', currentReel.id), data);
      } else {
        const { id: _id, ...data } = currentReel;
  ...
  if (!isAdmin) return (
    <div className="pt-40 text-center max-w-md mx-auto px-4">
      ...
      <p className="text-gray-500">The editorial dashboard is only accessible to authorized administrators. If you believe this is an error, please contact the system administrator.</p>
      <Link to="/" className="inline-block mt-8 text-sm font-bold underline">Return to Home</Link>
    </div>
  );

  const inputCls = "w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-black outline-none transition-all";

  return (
    <div className="pt-32 pb-40 min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <motion.div variants={fadeUp} initial="hidden" animate="visible" className="flex justify-between items-center mb-10">
          <div>
          ...
          <button
            onClick={() => {
              if (activeTab === 'blogs') { setCurrentPost(emptyPost()); setIsEditingPost(true); }
              else { setCurrentReel(emptyReel()); setIsEditingReel(true); }
            }}
        ...
        <div className="flex gap-2 mb-8 bg-white rounded-2xl p-1.5 border border-gray-100 shadow-sm w-fit">
          {(['blogs', 'reels'] as const).map(tab => (
            <button key={tab}
              ...
              className={cn("px-6 py-2.5 rounded-xl font-bold text-sm capitalize transition-all",
        ...
        {activeTab === 'blogs' && (
          isEditingPost ? (
            <motion.div variants={scaleIn} initial="hidden" animate="visible" className="bg-white rounded-3xl p-8 shadow-xl border border-gray-100">
              <form onSubmit={handleSavePost} className="space-y-8">
                <div className="space-y-4">
                  <label className="block text-sm font-bold text-gray-700">Headline</label>
                  <input type="text" required value={currentPost.title} onChange={e => setCurrentPost({ ...currentPost, title: e.target.value })} className={inputCls} placeholder="Enter a compelling headline..." />
                </div>
                <div className="space-y-4">
                  <label className="block text-sm font-bold text-gray-700">Excerpt</label>
                  <textarea required value={currentPost.excerpt} onChange={e => setCurrentPost({ ...currentPost, excerpt: e.target.value })} className={`${inputCls} h-24`} placeholder="A brief summary for the feed..." />
                </div>
                <div className="space-y-4">
                  <label className="block text-sm font-bold text-gray-700">Content (Markdown)</label>
                  <textarea required value={currentPost.content} onChange={e => setCurrentPost({ ...currentPost, content: e.target.value })} className={`${inputCls} h-96 font-mono text-sm`} placeholder="# Start writing your blog..." />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <div className="space-y-4">
                    <label className="block text-sm font-bold text-gray-700">Cover Image URL</label>
                    <input type="text" value={currentPost.coverImage} onChange={e => setCurrentPost({ ...currentPost, coverImage: e.target.value })} className={inputCls} />
                  </div>
                  <div className="space-y-4">
                    <label className="block text-sm font-bold text-gray-700">Status</label>
                    <select value={currentPost.status} onChange={e => setCurrentPost({ ...currentPost, status: e.target.value as 'draft' | 'published' })} className={inputCls}>
                  ...
                  <div className="space-y-4">
                    ...
                    <input type="text" value={currentPost.readingTime} onChange={e => setCurrentPost({ ...currentPost, readingTime: e.target.value })} className={inputCls} />
                  </div>
                </div>
                <div className="flex justify-end space-x-4 pt-8">
                  <button type="button" onClick={() => setIsEditingPost(false)} className="px-8 py-3 rounded-full font-bold text-gray-500 hover:text-black transition-colors">Cancel</button>
                  <button type="submit" className="bg-black text-white px-10 py-3 rounded-full font-bold hover:bg-gray-900 transition-all flex items-center space-x-2">
                ...
                <tbody className="divide-y divide-gray-50">
                  {posts.map(post => (
                    <tr key={post.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-8 py-6">
                        <span className={cn("inline-flex items-center space-x-1 px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest",
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end space-x-2">
                          <button onClick={() => { setCurrentPost(post); setIsEditingPost(true); }} className="p-2 hover:bg-gray-200 rounded-lg transition-colors text-gray-600"><Edit2 className="w-4 h-4" /></button>
                          <button onClick={() => handleDeletePost(post.id)} className="p-2 hover:bg-red-100 rounded-lg transition-colors text-red-600"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            <motion.div variants={scaleIn} initial="hidden" animate="visible" className="bg-white rounded-3xl p-8 shadow-xl border border-gray-100">
              <form onSubmit={handleSaveReel} className="space-y-8">
                <div className="space-y-4">
                  ...
                  <input type="text" required value={currentReel.title} onChange={e => setCurrentReel({ ...currentReel, title: e.target.value })} className={inputCls} placeholder="e.g. Understanding Tort Reform in 60 Seconds" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <label className="block text-sm font-bold text-gray-700">Views (display text)</label>
                    <input type="text" required value={currentReel.views} onChange={e => setCurrentReel({ ...currentReel, views: e.target.value })} className={inputCls} placeholder="e.g. 124k" />
                  </div>
                  <div className="space-y-4">
                    <label className="block text-sm font-bold text-gray-700">Order (0 = first)</label>
                    <input type="number" value={currentReel.order} onChange={e => setCurrentReel({ ...currentReel, order: parseInt(e.target.value) })} className={inputCls} />
                  </div>
                </div>
                <div className="space-y-4">
                  <label className="block text-sm font-bold text-gray-700">Thumbnail Image URL</label>
                  <input type="text" required value={currentReel.img} onChange={e => setCurrentReel({ ...currentReel, img: e.target.value })} className={inputCls} placeholder="https://..." />
                  {currentReel.img && (
                ...
                <div className="space-y-4">
                  <label className="block text-sm font-bold text-gray-700">Video / Instagram Reel URL</label>
                  <input type="text" required value={currentReel.video} onChange={e => setCurrentReel({ ...currentReel, video: e.target.value })} className={inputCls} placeholder="https://www.instagram.com/reel/ABC123/ or direct .mp4 URL" />
                  <p className="text-xs text-gray-400">Paste your Instagram reel link (e.g. instagram.com/reel/…) and it will be embedded automatically.</p>
                </div>
                <div className="flex justify-end space-x-4 pt-8">
                  <button type="button" onClick={() => setIsEditingReel(false)} className="px-8 py-3 rounded-full font-bold text-gray-500 hover:text-black transition-colors">Cancel</button>
                  <button type="submit" className="bg-black text-white px-10 py-3 rounded-full font-bold hover:bg-gray-900 transition-all flex items-center space-x-2">
                    ...
                    {reels.map(reel => (
                      <tr key={reel.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-8 py-5 text-right">
                          <div className="flex justify-end space-x-2">
                            <button onClick={() => { setCurrentReel(reel); setIsEditingReel(true); }} className="p-2 hover:bg-gray-200 rounded-lg transition-colors text-gray-600"><Edit2 className="w-4 h-4" /></button>
                            <button onClick={() => handleDeleteReel(reel.id)} className="p-2 hover:bg-red-100 rounded-lg transition-colors text-red-600"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
